class duoSecurity {
    ...
    [bool] set_duoUserGroupAddMembershipByUserIDAndGroupID ([string] $userID, [string] $groupID) {
        ...
        $statusCode = ''
        Invoke-RestMethod @generatedRequest -StatusCodeVariable 'statusCode'
        if ($statusCode -eq '200') {
            return $true
        }
        else {
...
$users = $duoSecurity.get_duoUserByUsername('someusername')
...
$user = $duoSecurity.get_duoUserByUsername('SomeUser@mydomain.edu')
